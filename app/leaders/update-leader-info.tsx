...
const UpdateLeaderInfo = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      const fetchPerson = async () => {
        setLoading(true);
        const docRef = doc(db, 'slider', code as string);
  ...
  const deleteOldImages = async () => {
    try {
      const imagesRef = ref(storage, 'frgians/leaders'); // The folder where images are stored
  ...
  const UpdatePersonHandler = async () => {
    ...
    if (isNewImageSelected) {
      ...
      const storageRef = ref(storage, `frgians/leaders/${fileName}`);
  ...
  const updatePersonDetails = async (imgUrl: string) => {
    ...
    if (Object.keys(updatedFields).length === 3) {
      ...
      Toast.show({
        ...
        position: 'bottom',
        visibilityTime: 2000, // SHORT equivalent (ToastAndroid.SHORT ≈ 2 seconds)
      });
    ...
    // ToastAndroid.show('Leader updated successfully!', ToastAndroid.LONG);
    Toast.show({
      type: 'success', // can also be 'info' or 'error'
      text1: 'Leader updated successfully!',
      position: 'bottom',
          ...
          <SafeAreaView style={{ flex: 1 }}>
            ...
            >
              ...
              <View className='flex items-center'>
                ...
                >
                  <Image
                    source={!image ? images.FRGiansBG : { uri: image }}
                    className='w-48 h-48 rounded-2xl bg-darkest'
                  />
                </TouchableOpacity>
                <Text
              ...
              <View className='gap-2 mt-3 flex items-center'>
                <View className='flex flex-row items-center w-[90%] justify-between'>
                  ...
                  <TextInput
                    className='w-[80%] pl-3 py-3 rounded-lg bg-coTitle border-2 border-zinc-300'
                ...
                <View className='flex flex-row items-center w-[90%] justify-between'>
                  ...
                  <TextInput
                    className='w-[80%] h-28 pl-3 rounded-lg bg-coTitle border-2 border-zinc-300'
...
export default UpdateLeaderInfo;
